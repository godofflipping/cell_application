...
class Ui_MainWindow(QCsMainWindow):
    ...
    def startOver(self):
        ...
        self.start_config = {"algorithm": self.algo_name, "cells": {}, "meta_data": {"image": [], "coords": [], "masks": []}}
...
###############################################################################
    ...
    def changeCell(self, item):
        ...
        self.current_cell = QPixmap(self.path_to_dir + self.current_image_path).copy(x, y, width, height)
...
###############################################################################
    ...
    def getCellList(self, item):
        ...
        if item.text() in self.cells:
            ...
            for key in self.full_data['cells']:
                cell_index = int(key.split('_')[1]) - 1
                cell_class = max(self.full_data['cells'][key]["probability"], key = self.full_data['cells'][key]["probability"].get)
                cell_proba = self.full_data['cells'][key]["probability"][cell_class]
                cell_names[cell_index] += (' ' + cell_class + ' ' + str(cell_proba))
...
###############################################################################
    ...
    def changeSegmentMode(self, item):
        try:
            ...
            if self.is_segmented and self.is_algo_chosen:
                ...
                self.whole_img.setPhoto(QPixmap(self.path_to_dir + self.current_image_path))
        except:
            pass
    ...
    def openData(self):
        ...
        if not os.path.exists(json_name):
            ...
            file.write('{"algorithm": self.algo_name, "cells": {}, "meta_data": {"image": [], "coords": [], "masks": []}}')
        else:
            self.openJson()
    ...
    def saveData(self):
        ...
        self.full_data['cells'] = self.start_config['cells']
...
###############################################################################
    ...
    def addCellItem(self, item):
        ...
        if self.is_save_mode and key in self.full_data['cells'].keys():
            prev_value = self.full_data['cells'][key]["path"]
        ...
        self.full_data['cells'][key] = meta_data
   
        self.data_images[(key, self.full_data['cells'][key]["path"])] = self.current_cell
    ...
    def changeMode(self, item):
        try:
            ...
            if self.is_dataset_mode:
                ...
                if key in self.full_data['cells'].keys():
                    for cell_class in self.classes:
                        self.proba_write.addItem(cell_class + " (" + \
                        str(self.full_data['cells'][key]["probability"][cell_class]) + ")")
                else:
...
###############################################################################
    ...
    def getFullInfo(self, item):
        ...
        for cell_class in self.classes:
            ...
            for key in self.full_data['cells'].keys():
                if cell_class == max(self.full_data['cells'][key]["probability"], key = self.full_data['cells'][key]["probability"].get):
                    count_class[cell_class] += 1
        
        counter = 1
        for cell_class in self.classes:
    
            if len(self.full_data['cells']) != 0:
...
###############################################################################        
    ...
    def getProba(self, item):
        key = self.img_list.currentItem().text().split('.')[0] + '_' + \
              item.text().split(' ')[1]
        if key not in self.full_data['cells'].keys():
